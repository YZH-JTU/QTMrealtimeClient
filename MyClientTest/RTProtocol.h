...
public:
    ...
    bool       Connect(char* pServerAddr, unsigned short nPort, unsigned short* pnUDPServerPort = NULL, int nMajorVersion = MAJOR_VERSION,
    ...
    bool       StartRTOnFile();
    bool       StopCapture();
    bool       LoadCapture(char* pFileName);
    bool       SaveCapture(char* pFileName, bool bOverwrite, char* pNewFileName = NULL, int nSizeOfNewFileName = 0);
    bool       LoadProject(char* pFileName);
    bool       Reprocess();
    ...
    const char*  Get3DBoneFromName(unsigned int boneIndex);
    const char*  Get3DBoneToName(unsigned int boneIndex);
    ...
    const char*  GetGazeVectorName(unsigned int nGazeVectorIndex);
    ...
    const char*  GetAnalogLabel(unsigned int nDeviceIndex, unsigned int nChannelIndex);
    const char*  GetAnalogUnit(unsigned int nDeviceIndex, unsigned int nChannelIndex);
    ...
    bool         GetForcePlateCalibrationMatrix(unsigned int nPlateIndex, float fvCalMatrix[12][12], unsigned int* rows, unsigned int* columns);
...
private:
    ...
    void AddXMLElementBool(CMarkup* oXML, const _TCHAR* tTag, const bool* pbValue,
		const _TCHAR* tTrue = _T("True"), const _TCHAR* tFalse = _T("False"));
    ...
    bool CompareNoCase(std::string tStr1, const _TCHAR* tStr2);
...
private:
    ...
    std::vector< std::string >    mvsGazeVectorSettings;
