...
bool CRTProtocol::Connect(char* pServerAddr, unsigned short nPort, unsigned short* pnUDPServerPort,
...
bool CRTProtocol::GetCurrentFrame(unsigned int nComponentType, const char* selectedAnalogChannels)
{
    ...
    if (GetComponentString(pCommandStr + (int)nCommandStrSize, sizeof(pCommandStr) - (int)nCommandStrSize, nComponentType, selectedAnalogChannels))
    {
...
bool CRTProtocol::StreamFrames(EStreamRate eRate, unsigned int nRateArg, unsigned short nUDPPort, const char* pUDPAddr,
                               unsigned int nComponentType, const char* selectedAnalogChannels)
{
    char pCommandStr[256];
...
bool CRTProtocol::SaveCapture(char* pFileName, bool bOverwrite, char* pNewFileName, int nSizeOfNewFileName)
{
    char tTemp[100];
    char pResponseStr[256];
...
bool CRTProtocol::ConvertRateString(const char* pRate, EStreamRate &eRate, unsigned int &nRateArg)
{
    ...
    else if (rateString.compare(0, rateString.size(), _T("frequency:"), 10) == 0)
    {
    ...
    if (rateString.compare(0, rateString.size(), _T("frequencydivisor:"), 17) == 0)
    {
...
bool CRTProtocol::GetComponentString(char* pComponentStr, int nComponentStrLen, unsigned int nComponentType, const char* selectedAnalogChannels)
{
    pComponentStr[0] = 0;
    ...
    {
        if ((nComponentType & cComponentAnalog) && selectedAnalogChannels != NULL && strlen(selectedAnalogChannels) > 0)
        {
    ...
    {
        if ((nComponentType & cComponentAnalogSingle) && selectedAnalogChannels != NULL)
        {
...
bool CRTProtocol::ReadCameraSystemSettings()
{
    ...
    if (!ReceiveRTPacket(eType, true))
    {
        ...
        sprintf_s(maErrorStr, sizeof(maErrorStr), "No response received. Expected XML packet. %s", pTmpStr);
        return false;
    }
    ...
    _TCHAR* processings[3] = { (_TCHAR*)"Processing_Actions", (_TCHAR*)"RealTime_Processing_Actions", (_TCHAR*)"Reprocessing_Actions" };
...
void CRTProtocol::GetSystemSettings(
    ...
    EProcessingActions &eProcessingActions, EProcessingActions &eRtProcessingActions, EProcessingActions &eReprocessingActions)
{
...
bool CRTProtocol::GetCameraSettings(
    ...
    bool         &bUnderwater, bool &bSupportsHwSync, unsigned int &nSerial, ECameraMode  &eMode)
{
...
bool CRTProtocol::GetCameraPosition(
    unsigned int nCameraIndex, SPoint &sPoint, float fvRotationMatrix[3][3])
{
    if (nCameraIndex < msGeneralSettings.vsCameras.size())
    {
...
bool CRTProtocol::GetCameraOrientation(
    unsigned int nCameraIndex, int &nOrientation)
{
    if (nCameraIndex < msGeneralSettings.vsCameras.size())
    {
...
const char* CRTProtocol::GetAnalogLabel(unsigned int nDeviceIndex, unsigned int nChannelIndex)
{
    if (nDeviceIndex < mvsAnalogDeviceSettings.size())
    {
...
const char* CRTProtocol::GetAnalogUnit(unsigned int nDeviceIndex, unsigned int nChannelIndex)
{
    if (nDeviceIndex < mvsAnalogDeviceSettings.size())
    {
                                       ...
                                       unsigned int &nChannelNumber, float &fConversionFactor)
{
    if (nPlateIndex < msForceSettings.vsForcePlates.size())
    {
        ...
        {
            sprintf_s(maErrorStr, sizeof(maErrorStr), "No channel index % on force plate index %d.", nChannelIndex, nPlateIndex);
        }
    }
    else
    {
        sprintf_s(maErrorStr, sizeof(maErrorStr), "No force plate index %d.", nPlateIndex);
    }
    return false;
}


bool CRTProtocol::GetForcePlateCalibrationMatrix(unsigned int nPlateIndex, float fvCalMatrix[12][12], unsigned int* rows, unsigned int* columns)
{
    if (nPlateIndex < msForceSettings.vsForcePlates.size())
    {
    ...
    sprintf_s(maErrorStr, sizeof(maErrorStr), "No force plate calibration matrix for camera index %d.", nPlateIndex);
    return false;
}
                                 ...
                                 float &fCropLeft, float &fCropTop, float &fCropRight, float &fCropBottom)
{
    ...
    sprintf_s(maErrorStr, sizeof(maErrorStr), "No camera image settings for camera index %d.", nCameraIndex);
    return false;
}
    ...
    const EProcessingActions* peProcessingActions, const EProcessingActions* peRtProcessingActions, const EProcessingActions* peReprocessingActions)
{
    ...
    const _TCHAR* processings[3] = { "Processing_Actions", "RealTime_Processing_Actions", "Reprocessing_Actions" };
...
bool CRTProtocol::SetCameraVideoSettings(
    const unsigned int  nCameraID,        const ECameraVideoMode* peVideoMode,
