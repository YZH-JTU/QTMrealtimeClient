...
bool CInput::ReadOperation(EOperation &eOperation)
{
    ...
    {
        printf("7 : View Settings (General, 3D, 6DOF, GazeVector, Analog, Force & Image)\n");
    }
    if (mnMajorVersion > 1 || mnMinorVersion > 6)
    {
...
bool CInput::ReadDataComponents(unsigned int &nComponentType, char* selectedAnalogChannels, int selectedAnalogChannelsLen)
{
    bool bNoComponentSelected = true;
    
    while (bNoComponentSelected)
    {
        nComponentType = ReadDataComponent(false);
        ...
        if (nComponentType == 0xffffffff)
        {
            ...
            do
            {
                nAddType = ReadDataComponent(true);
    ...
    if (nComponentType == CRTProtocol::cComponentAnalog || nComponentType == CRTProtocol::cComponentAnalogSingle && selectedAnalogChannels != NULL)
    {
...
unsigned int CInput::ReadDataComponent(bool bMultiple)
{
    int c;
    ...
    {
        printf("Add Component to multiple selection:\n\n");
    }
    printf("1: 2D (Default)\n");
    ...
    if (!bMultiple)
    {
        printf("h: Multiple Components\n");
        printf("i: All Components\n");
        printf("Q: Quit\n\n");
    }
    else
    {
        printf("\nSelect component to add. Enter to stop selection.\n\n");
    }
    ...
    switch (c)
    {
        ...
        case 'h' :
            ...
            break;
        case 'i' :
            nComponentType = CRTProtocol::cComponentAll;
            break;
        case 'q' :
        case 'Q' :
            nComponentType = 0;
            break;
        default:    
            nComponentType = bMultiple ? 0 : CRTProtocol::cComponent2d;
            break;
    }
...
{
    const _TCHAR* processings[3] = { "Processing Actions", "Real-time Processing Actions", "Reprocessing Actions" };
...
void CInput::ReadCameraSettings(unsigned int &nCameraId,        int   &nMode,            int   &nVideoMode,
                                ...
                                float        &fMarkerExposure,  float &fMarkerThreshold, int   &nRotation)
{
    ...
    if (nMode == 2)
    {
        printf("Enter Video Mode (Default Custom):\n");
        printf("  1 : Custom\n");
        printf("  2 : 1080p 24Hz\n");
        printf("  3 : 720p 25Hz\n");
        printf("  4 : 720p 50Hz\n");
        ...
        printf("  b : 480p 120Hz\n");
        printf("Select 1 - b : ");
        nVideoMode = ReadChar('1', true) - '0' - 1;
        if (nVideoMode == 'a' - '0' - 1 || nVideoMode == 'b' - '0' - 1) {
            nVideoMode -= 39;
        }
        if (nVideoMode < 0 || nVideoMode > 10)
        {
            nVideoMode = 0;
        }
